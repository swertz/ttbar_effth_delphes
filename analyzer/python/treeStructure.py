...
class MISBox:
	...
	def __init__(self, parent = None, cfg = None):
		...
		if self.parent is not None:
			self.cfg = self.parent.cfg # PConfig object
	...
	def print(self):
		if self.isEnd:
			print "=== Box " + self.name + ", level" + str(self.level) + ":"
				for proc in self.cfg.procCfg:
					print "==== Process " + proc["name"] + ": " + proc["entries"] + " MC events, " + proc["yield"] + " expected events."
				print "\n"
		else:
...
class MISTree:
	...
	def __init__(self, cfg):
		...
		self.firstBox.cfg = self.cfg # PConfig object
