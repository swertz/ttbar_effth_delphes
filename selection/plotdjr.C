...
//  TCut mult3 = "GenEvent.nMEPartonsFiltered()==3";
  ...
  hmult0->SetLineColor(kBlue+1);
  hmult1->SetLineColor(kRed+1);
  hmult2->SetLineColor(kGreen+2);
  ...
  TLegend *legend=new TLegend(0.67, 0.87-3*0.06, 0.87, 0.87);
  setlegend(legend, hall, hmult0, hmult1, hmult2/*, hmult3*/);
  
  //gStyle->SetOptStat(0);

  hall->SetStats(kFALSE);
  hmult0->SetStats(kFALSE);
  hmult1->SetStats(kFALSE);
  hmult2->SetStats(kFALSE);
...
void plotdjr() {
  ...
  TString outfile("plot_OG_DJR_qCut50.root");
  ...
  for(int i = 0; i < 50; ++i){
    ...
    path << "../OG_qCut50/condor/output/output_pythia8_" << i << ".root";
    std::cout << "Adding " << path.str() << std::endl;
    tree->Add(path.str().c_str());
  }
  ...
  pad[0]->cd();
  makeplot("djr0", tree,weight, "log10(GenEvent.DJRValues_[0])", "log(DJR 0->1)", nbins, djrmin, djrmax);
  pad[1]->cd();
  makeplot("djr1", tree,weight, "log10(GenEvent.DJRValues_[1])", "log(DJR 1->2)", nbins, djrmin, djrmax);
  pad[2]->cd();
  makeplot("djr2", tree,weight, "log10(GenEvent.DJRValues_[2])", "log(DJR 2->3)", nbins, djrmin, djrmax);
  //pad[3]->cd();
